...
- **CORE = VOO + a 10% thematic slice (SPCX).** All capital not committed to a signal position is held in the core; cash is never the resting state — after every run, uninvested headroom is swept into VOO. Maintain the thematic slice at **10% of the cap in SPCX** (SpaceX — owner's standing thematic pick since its 2026 IPO): buy it up to target from headroom when below, but never sell VOO to top it up and never let it exceed 10% at cost. The slice is buy-and-hold like the rest of the core — never stop-managed, never sold on score; VOO is only sold to fund a qualifying signal buy.
